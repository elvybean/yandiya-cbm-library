...
class Item:
    ...
    def get_cbm(self):
        return set_to_decimal((self.width * self.height * self.depth) / 1000000000, self.number_of_decimals)
...
class Bin:
    ...
    def string(self):
        return "%s(%sx%sx%s, max_weight:%s) cbm(%s)" % (
...
class Packer:
    ...
    def pack(
        ...
        self.bins.sort(
            key=lambda bin: bin.get_cbm(), reverse=bigger_first
        )
        self.items.sort(
            key=lambda item: item.get_cbm(), reverse=bigger_first
        )
